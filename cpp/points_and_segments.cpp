...
int searchPoint(segment &seg, const vector<int> &points, int target)
{
...
{
  // Find start point
  int start = searchPoint(seg, points, seg.first);

  // Find end point
  int end = searchPoint(seg, points, seg.second + 1) - 1;
...
vector<int> fast_count_segments(vector<int> starts, vector<int> ends, vector<int> points)
{
  ...
  for_each(cnt.begin(), cnt.end(), [](int &x)
           { x = 0; });
